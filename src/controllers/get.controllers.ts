...
export const getLote = async (req:Request, res:Response):Promise<Response>=>{
    ...
    try{ //Con el try realizamos que si alla un error devuelve una respuesta para que no se quede esperado una respuesta 
        ...
        const lote:QueryResult = await con.query('SELECT row_to_json(fc) FROM ( SELECT array_to_json(array_agg(f)) As Datos FROM (SELECT ST_AsGeoJSON(lg.area)::json As geometry, row_to_json((loteid,estado,calleid)) As properties FROM lote As lg) As f) As fc');
...
export const getManzano = async (req:Request, res:Response):Promise<Response>=>{
    ...
    try{ //Con el try realizamos que si alla un error devuelve una respuesta para que no se quede esperado una respuesta
        ...
        const con = await connet();
        const lote:QueryResult = await con.query('SELECT * FROM public.manzano ORDER BY manzanoid ASC ');
        return res.status(200).json(lote.rows);
    }
